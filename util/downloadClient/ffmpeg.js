...
module.exports = async () => new Promise(async res => {
    ...
    ghRequest().then(async r => {   
        ...
        if((await require(`../updateAvailable/ffmpeg`).check(r)) == false) {
            ...
            let downloadFFmpeg, downloadFFprobe, ffmpegVer, platformname;

            if(process.platform == `win32`) {
                let archname = process.arch.slice(process.arch.startsWith(`x`) ? 1 : 0);
                platformname = process.platform.replace(/[0-9]/g, ``) + archname;

                console.log(`WINDOWS PLATFORM NAME: "${platformname}"`)

                ffmpegVer = `8.1`;
                console.log(`LOOKING FOR ${`ffmpeg-n${ffmpegVer}-latest-${platformname}-gpl-shared-${ffmpegVer}.zip`}`)
                downloadFFmpeg = downloads.find(d => d.name == `ffmpeg-n${ffmpegVer}-latest-${platformname}-gpl-shared-${ffmpegVer}.zip`);
                downloadFFprobe = null;
            } else {
                downloadFFmpeg = downloads.find(d => d.name == `ffmpeg-${process.platform}-${process.arch}`);
                downloadFFprobe = downloads.find(d => d.name == `ffprobe-${process.platform}-${process.arch}`);
            }

            if(!downloadFFmpeg || (!downloadFFmpeg && !downloadFFprobe)) {
                ...
                if(downloadFFprobe) console.log(`Found target FFprobe: ${downloadFFprobe.name} / ${downloadFFprobe.size} size; downloading from "${downloadFFprobe.browser_download_url}"`);
                ...
                if(process.platform == `win32`) {
                    ...
                    const dlPath = require('path').join(downloadPath, downloadFFmpeg.name);

                    require(`../downloadClientTo`)({
                        ws,
                        version,
                        str: `FFmpeg (${platformname}, GPL v${ffmpegVer})`,
                        ...
                        const zip = new require('adm-zip')(require('path').join(downloadPath, downloadFFmpeg.name));

                        zip.extractAllTo(downloadPath, true);
                        
                        if(await pfs.existsSync(dlPath)) await pfs.rmSync(dlPath);
